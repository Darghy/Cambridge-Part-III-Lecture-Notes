...
\begin{center}
    \begin{tikzpicture}
        ...
        \draw[->] (axis cs: -1.5,0) -- (axis cs: 1.5,0) node[below right] {$x$};
        ...
        \else
            \draw[->] (axis cs: 0,0) -- (axis cs: 0,3) node[above left] {$y$};
        \fi
    
        % Draw semicircle
        \addplot[domain=0:180, samples=100, black] ({cos(x)}, {sin(x)});
        \node[black] at (axis cs: 1.1, 0.9) {$x^2 + y^2 = 1$};
    
        % Draw lines x=1/2 and x=-1/2 with cutoff
        \ifnum\pdfstrcmp{\pgfkeysvalueof{/pgfplots/xmin}}{-0.2}>0
            \draw[dashed] (axis cs: 0.5,-0.2) -- (axis cs: 0.5,3) node[above] {$x=\frac{1}{2}$};
            \draw[dashed] (axis cs: -0.5,-0.2) -- (axis cs: -0.5,3) node[above] {$x=-\frac{1}{2}$};
        \else
            \draw[dashed] (axis cs: 0.5,0) -- (axis cs: 0.5,3) node[above] {$x=\frac{1}{2}$};
            \draw[dashed] (axis cs: -0.5,0) -- (axis cs: -0.5,3) node[above] {$x=-\frac{1}{2}$};
        \fi
