...
\begin{proof}
    ...
    \end{itemize}
\end{proof}
If $K$ is complete, then $\mathcal{O}_K$ is complete with respect to $\pi^r\mathcal{O}_K$ for any $r\ge 1$. We fix a minimal Weierstrass equation for $E/K$. This gives rise to a formal group $\widehat{E}$ over $\mathcal{O}_K$. Take $R = \mathcal{O}_K$, $I = \pi^r \mathcal{O}_K$ for $r\ge 1$ in Lemma \ref{lemma8.2} to get $\widehat{E}(\pi^r \mathcal{O}_K) = \{(x,y) \in E(K) \mid -\frac{x}{y},-\frac{1}{y} \in \pi^r \mathcal{O}_K\}$.
