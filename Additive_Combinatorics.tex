...
\begin{proof}
    ...
    \end{align*}
    TODO: finish this expression (in the lecture, she misses the expectation in the last line, but it should be there). So for each $y \in G$, $||\sum_{i=1}^{k} Z_i(y)||_{L^p(\mathbb{P})}^p = O \left(p^{p/2} k^{\frac{p}{2}-1} \mathbb{E}\sum_{i=1}^{k} \left|Z_i(y)\right|^p\right)$.
...
\end{proof}
\begin{theorem}[due to Schoen and Shlvedov]\label{theorem3.10}
